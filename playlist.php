...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['azione'])) {
    ...
    if ($_POST['azione'] === 'riordina') {
        ...
        foreach ($ids as $pos => $id) {
            $db->prepare('UPDATE playlist_items SET ordine=? WHERE id=?')->execute([$pos, intval($id)]);
        }
        echo 'ok'; exit;
    }
...
<div class="container" style="display:grid; grid-template-columns:280px 1fr; gap:24px;">
    ...
    <div>
        <?php if ($playlist_attiva):
            ...
            $durata_totale = array_sum(array_map(fn($it) => $it['tipo'] === 'video' ? ($it['durata'] ?: 30) : $it['durata'], $items_attivi));
        ?>
        <div class="box">
            ...
            <?php else: ?>
                ...
                <div data-id="<?php echo $item['id']; ?>"
                     style="display:flex; align-items:center; gap:12px; padding:10px;
                            ...
                            opacity:<?php echo $opacita; ?>; position:relative; cursor:default;">

                    <div class="drag-handle" title="Trascina per riordinare"
                         style="font-size:18px; color:#1a4a7a; cursor:grab; padding:0 4px; user-select:none; line-height:1;">
                        ⠿
                    </div>

                    <div class="item-num" style="font-size:14px; font-weight:bold; color:#e94560; width:20px; text-align:center; flex-shrink:0;">
                        <?php echo $i + 1; ?>
                    </div>
...
if (lista) {
    Sortable.create(lista, {
        ...
        onEnd: function() {
            ...
            fetch('/playlist.php', { method:'POST', body: fd })
                .then(r => r.text())
                .then(t => {
                    if (t === 'ok') {
