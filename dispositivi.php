    ...
    <table style="width:100%;border-collapse:collapse;">
        <thead>
            <tr style="border-bottom:1px solid rgba(255,255,255,0.07);">
                ...
                <th style="padding:12px 16px;text-align:left;font-size:11px;color:var(--sg-muted);font-weight:600;text-transform:uppercase;letter-spacing:1px;">Tipo</th>
        ...
        <tr class="dev-row" data-search="<?= strtolower(htmlspecialchars($d['nome'].' '.$d['club'].' '.$d['token'])) ?>"
            ...
            <td style="padding:12px 16px;"><?= tipoDisplayBadge($d['tipo_display']??'tv', $tipi_display) ?></td>
...
<div id="vista-card" style="display:none;">
    ...
    <div class="dev-row box" data-search="<?= strtolower(htmlspecialchars($d['nome'].' '.$d['club'].' '.$d['token'])) ?>"
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
            ...
            <?= tipoDisplayBadge($d['tipo_display']??'tv', $tipi_display) ?>
        </div>
...
<div style="max-width:580px;">
    ...
    <div class="box">
        <form method="POST" id="dev-form">
            ...
            <select name="tipo_display">
                <option value="tv"    <?= ($dev['tipo_display']??'tv')==='tv'   ?'selected':'' ?>>📺 TV (1920×1080)</option>
                <option value="led"   <?= ($dev['tipo_display']??'')==='led'    ?'selected':'' ?>>💡 Insegna LED (1720×320)</option>
                <option value="totem" <?= ($dev['tipo_display']??'')==='totem'  ?'selected':'' ?>>🗼 Totem (256×768)</option>
            </select>
...
document.querySelectorAll('.tipo-card').forEach(function(card) {
    card.addEventListener('click', function() {
        ...
        if (!label) return;
        var radio = label.querySelector('.tipo-radio');
        if (!radio) return;
        radio.checked = true;
        ...
        var c = colors[radio.value] || '#3b82f6';
